sap.ui.define([
	...
	return BaseObject.extend("employeesdashboard.model.GroupSortState", {
		 ...
		 * @alias employeesdashboard.model.GroupSortState
		 */
		...
		/**
		 * Sorts by GivenName, or by Attrition
		 *
		...
		/**
		 * Groups by Attrition, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "Attrition") {
				...
				this._oViewModel.setProperty("/sortBy", "Attrition");

				aSorters.push(
					new Sorter("Attrition", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "GivenName");
			}
